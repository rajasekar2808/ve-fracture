...
## load the mesh
basemeshname = 'beam_offset_notch_hole_fine'     ## name of the mesh
meshid = ''
...
## Dimensions in 'm'
thickness = .5/100; b= thickness;  ## thickness in z-direction 
L = 20/100 ; 
...
simulation_id = '_LF_v0_26_09_22_contd_'     ## give new id for performing simulation on same mesh with different setting (to save in a different folder)
expname = basemeshname+ simulation_id

### for outputfiles 
respath = '/scratch/OG2111300'
...
resume_path = r'/scratch/OG2111300/beam_offset_notch_hole_fine_LF_v0_26_09_22_/results/results_u_0.003124999999999995_.npz'
resume_path2 = r'/scratch/OG2111300/beam_offset_notch_hole_fine_LF_v0_26_09_22_/force_disp.npz'
resume_disp = 0.003124999999999995   ## can extract from the file name of resume_path
...
if mesh_in_cm:  mesh.xy = mesh.xy/100
nv = mesh.nvertices
nf =  mesh.ntriangles
...
alternedsolveroptions= {'abstole':1.e-8, 'reltole':1.e-5, 'deltadtol':1.e-3, 'max_iter': 10}
...
velocity = [i/1000 for i in [.1,1,.05]]        ## m/s
time_step = [1e-1,5e-3,5e-1] 
...
max_displacement_y = [i/1000 for i in [6,6,6]]    ## m    
stop_forces = [2.5,5,2.5]                                  #  N
...
videl_d0 =set()
dist_r = .7/100   ## for elements within a distance r from boundaries damage is enforced 0
...
while u_appl < max_uy:
    ...
    if res['ad_tim']:
        ...
        np.savez(file1,u=np.array(u).squeeze(),d=np.array(d).squeeze(),d_nodes=np.array(d_nodal).squeeze(),DT = np.array([DT1]),
